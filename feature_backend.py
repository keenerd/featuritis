...
def render_html(path, stateful):
    ...
    fp.write('<script>function smart_reload(p) {\n')
    fp.write('function tick() {if (navigator.onLine) {location.reload(true);} else {setTimeout(tick, p);}}\n')
    fp.write('setTimeout(tick, p);}\n')
    fp.write('smart_reload(60*1000); </script>\n')
    fp.write('</head><body>\n')
